...
# 11 "./EEPROM_24C32A.h" 2

# 1 "C:\\Program Files\\Microchip\\xc8\\v2.32\\pic\\include\\c99\\stdbool.h" 1 3
# 12 "./EEPROM_24C32A.h" 2
...
extern _Bool EERPOM_Write_byte(uint8_t eepromSlaveADDR, uint16_t regMapADDR, uint8_t dataByte){
    ...
    if(I2C_WriteMaster(eepromSlaveADDR)==1){
        return 1 ;
    }


    if(I2C_WriteWordMaster(0x0000 + regMapADDR)==1){
       return 1 ;
    }


    if(I2C_WriteMaster(dataByte)==1){
        return 1 ;
    }
...
extern _Bool EEPROM_Flush(uint8_t eepromSlaveADDR){
    ...
    if(I2C_WriteWordMaster(0x0000)==1){
       return 1 ;
    }

    for(unsigned int i=0 ; i<4096U;i++){

        if(I2C_WriteMaster(0xFF)==1){
            return 1 ;
        }
    }
...
extern _Bool EEPROM_Random_Read(uint8_t eepromRDSlaveADDR, uint16_t regMapADDR, uint8_t *pdataByteRead){
    ...
    if(I2C_WriteMaster(eepromRDSlaveADDR - 1U)==1){
        return 1 ;
    }
    ...
    if(I2C_WriteMaster(eepromRDSlaveADDR)==1){
        return 1 ;
    }


    *pdataByteRead = I2C_ReadMaster(0);
...
extern _Bool EEPROM_Sequential_Read(uint8_t eepromRDSlaveADDR, uint16_t regMapADDR, unsigned int numDataBytes, uint8_t *pdataByteList){
    ...
    for(unsigned int i=0 ;i<numDataBytes; i++){
        ...
        if(i==(numDataBytes - 1)){
            *pdataByteList++= I2C_ReadMaster(0);
            break;
        }else{
            *pdataByteList++= I2C_ReadMaster(1);
        }
    }
