...
# 1 "C:\\Program Files\\Microchip\\xc8\\v2.32\\pic\\include\\c99\\inttypes.h" 1 3
# 13 "C:\\Program Files\\Microchip\\xc8\\v2.32\\pic\\include\\c99\\inttypes.h" 3
# 1 "C:\\Program Files\\Microchip\\xc8\\v2.32\\pic\\include\\c99\\bits/alltypes.h" 1 3
# 13 "C:\\Program Files\\Microchip\\xc8\\v2.32\\pic\\include\\c99\\inttypes.h" 2 3
...
# 1 "./ConfBits_RealBoardPLL.h" 1
# 17 "./ConfBits_RealBoardPLL.h"
extern void OSCILATOR_Init(void);
# 19 "./RTOS.h" 2

# 1 "./INTERRUPT_Controller.h" 1
# 14 "./INTERRUPT_Controller.h"
# 1 "./TMR1.h" 1
# 80 "./TMR1.h"
static _Bool _highPriority;
...
extern void TMR1_Initialize(_Bool highPriority);
...
extern void Millis_Init(_Bool highPriority);
...
extern _Bool EUSART_Rx_Ready(void);
...
extern _Bool I2C_WriteMaster(uint8_t data);
extern _Bool I2C_WriteWordMaster(uint16_t data);
extern uint8_t I2C_ReadMaster(_Bool sendACK);
...
static volatile _Bool rtcTimeReady=0;
...
extern _Bool DS1307_Conf_Init(rtcDs1307Control_t *prtcDs1307Control);
extern _Bool DS1307_Set_TimeDate(rtcDs1307UserData_t *prtcDs1307UserData);
extern _Bool DS1307_Read_TimeDate(rtcDs1307UserData_t *prtcDs1307UserData);

extern void DS1307_ISR(void);
extern _Bool DS1307_GetISR_FlagStatus(void);
extern _Bool DS1307_ClearsISR_Flag(void);
...
# 1 "./pcf8574.h" 1
# 22 "./pcf8574.h"
extern _Bool pcf8574_SendData(uint8_t i2cSlaveAddr, uint8_t dataByte);
# 14 "./LCD_I2C.h" 2
# 39 "./LCD_I2C.h"
...
static uint8_t _fractConversionFactorIndex=0b11;
...
extern _Bool ds18b20_Configure(ds18b20Config_t *pds18b20Config);
extern _Bool ds18b20_Read_ROM(uint64_t *promCode);
extern _Bool ds18b20_Read_Raw_Temperature(uint16_t *rawTempValue);
extern _Bool ds18b20_Read_Real_Temperature(char *realTempStr, int8_t *integerTemp);
# 28 "./RTOS.h" 2

# 1 "./EEPROM_24C32A.h" 1
# 41 "./EEPROM_24C32A.h"
extern _Bool EERPOM_Write_byte(uint8_t eepromSlaveADDR, uint16_t regMapADDR, uint8_t dataByte);


extern _Bool EEPROM_Write_Page(uint8_t eepromSlaveADDR, uint16_t regMapADDR, char *pString);


extern _Bool EEPROM_Flush(uint8_t eepromSlaveADDR);


extern _Bool EEPROM_Random_Read(uint8_t eepromRDSlaveADDR, uint16_t regMapADDR, uint8_t *pdataByteRead);


extern _Bool EEPROM_Sequential_Read(uint8_t eepromRDSlaveADDR, uint16_t regMapADDR, unsigned int numDataBytes, uint8_t *pdataByteList);
# 29 "./RTOS.h" 2
# 44 "./RTOS.h"
typedef struct{
    uint16_t tID;
    uint16_t priority;
    uint32_t tPollRateMs;
    uint32_t tElapsedMs;
    uint16_t taskInList;
    void (*pvFuncTask)(void);
    char nameTask[10];
}TCB_t;
...
static char printLCDBuffer[16U];
 uint8_t eepromBufferRx[7];
 uint8_t eepromBufferTx[7];
...
static char Str_Real_temperature[12];
static int8_t temperatureInteger;
static _Bool timeAlarmEnable=0, temperatureHighAlarmEnable=0, temperatureLowAlarmEnable=0;
static _Bool actualTimeReadyToSaveRTC = 0;
static _Bool alarmsReadyToSaveEEPROM = 0;
static _Bool alarmsReadyToReadEEPROM = 0;
...
void Task2(void){
    ...
    if(DS1307_GetISR_FlagStatus() == 1)
    {
        DS1307_ClearsISR_Flag();


       if(DS1307_Read_TimeDate(&rtcDs1307UserData)==1){
            LATDbits.LATD0 = 1;
...
void Task3(void){
# 331 "main.c"
   if( (userAlarm.hours==rtcDs1307UserData.hours) &&
       (userAlarm.minutes==rtcDs1307UserData.minutes) &&
        (rtcDs1307UserData.minutes-userAlarm.minutes)<=userAlarm.timeAlarmDuration
     )
   {
   ...
   if(temperatureInteger>=(userAlarm.highTemperatureLimit - userAlarm.temperatureAlarmOffset))
   {
       ...
       temperatureHighAlarmEnable = 1;

   }else
   {

       temperatureHighAlarmEnable = 0;
   }
   ...
   if(temperatureInteger<=(userAlarm.lowTemperatureLimit + userAlarm.temperatureAlarmOffset))
   {
       ...
       temperatureLowAlarmEnable = 1;

   }else
   {

       temperatureLowAlarmEnable = 0;
   }
...
void Task4(void){
    ...
    if(timeAlarmEnable==1)
    {
        LATDbits.LATD6=~LATDbits.LATD6;

    }

    if(temperatureHighAlarmEnable==1)
    {
        LATDbits.LATD7=1;
    ...
    if(temperatureLowAlarmEnable==1)
    {
        LATCbits.LATC2=1;
...
void Task5(void){
    ...
    if(PORTDbits.RD2 ==0U)
    {
        ...
        while(PORTDbits.RD2 ==0U)
        {
            if(PORTDbits.RD4==0U)
            {
                vTaskDelay(15);
                while(PORTDbits.RD4==0U);
...
void Task6(void){
    ...
    if(actualTimeReadyToSaveRTC==1)
    {
        actualTimeReadyToSaveRTC= 0;


        if(DS1307_Set_TimeDate(&rtcDs1307UserData)==1){
            LATDbits.LATD0 = 1;
...
void Task7(void){
# 753 "main.c"
    if(alarmsReadyToSaveEEPROM==1)
    {
        alarmsReadyToSaveEEPROM=0;


        memcpy(&eepromBufferTx, &userAlarm, sizeof(userAlarm) );


        if(EEPROM_Write_Page(0xA0, 0x0000, eepromBufferTx)==1){
            LATDbits.LATD0 = 1;
            printf("\r\nError en el Bus I2C , reason 763!!");
            while(1);
        }

        alarmsReadyToReadEEPROM=1;
...
void Task8(void){
    ...
    if(alarmsReadyToReadEEPROM==1)
    {
        alarmsReadyToReadEEPROM=0;


       if(EEPROM_Sequential_Read(0xA1, 0x0000, 7U, eepromBufferRx)==1){
            LATDbits.LATD0 = 1;
