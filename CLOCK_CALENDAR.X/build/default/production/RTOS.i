...
extern uint8_t xTaskCreate(const char *pcName, uint16_t xPriority, const uint32_t pvTaskRateMs, void (*pvTaskCode)(void)){
    ...
    while(i <=8)
    {
        ...
        if (taskList[i].taskInList != 1)
        {
            ...
            taskList[i].tElapsedMs = Millis_Get();
...
# 75 "RTOS.c"
    while(1)
    {
        for(i = 0; i<=8; i++)
        {
            if(taskList[i].taskInList ==1)
            {
                if(Millis_Get()>=(taskList[i].tElapsedMs + taskList[i].tPollRateMs))
                {
                    taskList[i].tElapsedMs = Millis_Get();
...
extern void vHardwareInit(void){
    ...
    ds18b20Config.CONFIG = 0b00;
    ds18b20Config.TH = 100U;
    ds18b20Config.TL = 7U;
     ...
     )
   {
    ...
    rtcDs1307Control.rs=0b00;
    rtcDs1307Control.sqwe=1;
    rtcDs1307Control.out =0 ;


    if(DS1307_Conf_Init(&rtcDs1307Control)==1){
        LATDbits.LATD0 = 1;
    ...
    LCD_I2C_New_Char(1U,tempDegSymbol[0],tempDegSymbol[1],tempDegSymbol[2],tempDegSymbol[3],tempDegSymbol[4],tempDegSymbol[5],tempDegSymbol[6],tempDegSymbol[7]);
...
extern void vTaskDelay(uint32_t taskDelayMs){
    ...
    while((Millis_Get() - start)<=taskDelayMs){
# 246 "RTOS.c"
    }
}
